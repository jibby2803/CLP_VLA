...
def calculate_dataset_statistics(parquet_paths: list[Path]) -> dict:
    ...
    for le_modality in all_low_dim_data.columns:
        ...
        try:
            ...
            print("all_low_dim_data[le_modality][0] types:", type(all_low_dim_data[le_modality][0]))
            print("Decoding bytes data...")
            np_data = np.vstack(
                [np.asarray(decode_image_from_bytes_dict(x), dtype=np.float32) for x in all_low_dim_data[le_modality]]
            )
...
def decode_image_from_bytes_dict(bdict):
    """Given a dict with key 'bytes' that stores PNG/JPEG bytes, return HWC uint8 numpy array."""
    if not isinstance(bdict, dict):
        raise ValueError("expected dict")
    if 'bytes' not in bdict:
        raise ValueError("no 'bytes' key")
    b = bdict['bytes']
    if isinstance(b, memoryview):  # sometimes parquet gives memoryview
        b = b.tobytes()
    # bytes might already be a bytes object
